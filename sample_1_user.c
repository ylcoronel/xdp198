...
static const char *default_filename = "sample_1_kern.o";
static const char *default_progsec = "xdp1";
...
static void stats_poll(int map_fd)
{
	...
	while (1) {
        value = bpf_map_lookup_elem(map_fd, IPPROTO_UDP)
        if(value)
            printf(value, "\n");
	}
}
...
int main(int argc, char **argv)
{
	...
	stats_map_fd = find_map_fd(bpf_obj, "rxcnt");
	if (stats_map_fd < 0) {
	...
	map_expect.key_size    = sizeof(u32);
	map_expect.value_size  = sizeof(long);
